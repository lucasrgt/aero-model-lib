...
/**
 * Callback invoked by {@link Aero_AnimationPlayback} when playback crosses
 * a non-pose keyframe declared in the {@code .anim.json} {@code keyframes}
 ...
 * world.playSound}, spawn its own particles, etc.). Channel names are not
 * validated — they are pure routing strings — but the JSON schema
...
public interface Aero_AnimationEventListener {
     ...
     *                 to the consumer.
     * @param locator  optional bone name from the JSON ({@code null} when
     *                 the keyframe used the bare-string form). When set,
     *                 callers typically resolve it through
     *                 {@link Aero_AnimationPlayback#getAnimatedPivot} to
     *                 anchor sounds/particles to a specific part of the
     *                 moving mesh.
     * @param time     keyframe time in seconds, relative to clip start.
     */
    void onEvent(String channel, String data, String locator, float time);
}
