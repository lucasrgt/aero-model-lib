...
public class AeroRobotEntityRenderer extends EntityRenderer {
    ...
    public void render(Entity entity, double x, double y, double z,
        ...
        float gb = 1f - 0.65f * overheat;
        float r  = 1f;
        // During meltdown, pulse the tint between red (default) and yellow
        // (G boosted to ~0.85) at ~3 Hz so the chassis "flashes" hot — the
        // entity ticks 20×/s so a sin(t * 6π / 20) crests three times per
        // second.
        if (bot.isMeltdown()) {
            float t = (bot.age + partialTick) * (6f * (float) Math.PI / 20f);
            float pulse = 0.5f + 0.5f * (float) Math.sin(t);
            gb = gb + (0.85f - gb) * pulse;  // shift G+B from red toward yellow
        }
        ...
        Aero_MeshRenderer.setTint(1f, gb, gb);
        try {
